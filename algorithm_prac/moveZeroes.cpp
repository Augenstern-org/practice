...
/*
题目考查点：原地移动数组中所有 0 到末尾，同时保持非零元素相对顺序。
最优算法：双指针/快慢指针（in-place），一次遍历将非零元素写到前端，剩余位置填 0。时间 O(n)，空间 O(1)。
时间复杂度：O(n)。空间复杂度：O(1)。
实现可优化点：
 - 已有 Solution2 给出标准且高效实现，建议使用该实现并删除冗余或调试代码。
 - 第一版实现使用迭代器和多重判断，复杂且容易出错，且在大量连续零的场景中可能多次移动，增加常数开销。
学习方向：双指针技巧、原地算法思想、在需要保持相对顺序时的常见模式。
*/
