...
def check_list(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, list):
            ...
            if diff != 0:
                print(f'''{'*'*50}\n[ERROR]The returned list has an incorrect length! len(returned)={len(res)}, len(expected)={len(expected)}''')
    ...
    assert res==expected, '''Incorrect value returned!'''


def check_dict(res, expected, params=None, expl=''):
    if res != expected:
        if isinstance(res, dict):
            print(f'''{'*'*50}\n[ERROR] The returned dictionary is incorrect!\nReturned={res}\nexpected={expected}\n{'*'*50}''')
            for k in expected:
                if k not in res:
            ...
            print(f'''{'*'*50}\n[ERROR] The returned value is not a dictionary!\nReturned={type(res)}\nexpected={type(expected)}.{'*'*50}''')
    assert res==expected, '''Incorrect value returned!'''
...
def check_set(res, expected, expl=''):
    if not expl:
        expl = f'''{'*'*50}\n[ERROR] The returned set is incorrect!\nReturned={res}\nexpected={expected}\n{'*'*50}'''
    assert res == expected, expl
...
def runtests(tests, verbose=True, logfile='', stack_trace=False):
    if logfile:
        ...
        with open(logfile, newline='', encoding='utf8') as f:
            ...
            reader = csv.reader(f)
            for row in reader:
                if row[0] != 'test_personal_data_entry':
                    tot += float(row[1])
